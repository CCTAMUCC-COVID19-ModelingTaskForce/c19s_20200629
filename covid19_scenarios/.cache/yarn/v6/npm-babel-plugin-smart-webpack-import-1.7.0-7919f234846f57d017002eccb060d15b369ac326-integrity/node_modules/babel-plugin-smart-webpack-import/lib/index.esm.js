...
import { dirname, basename, extname, relative, sep } from 'path';
...
function processImport(path, state) {
  ...
  const requester = dirname(state.file.opts.filename); // Use only first part of template string as request part.
  ...
  if (!jsonContent.webpackChunkName) {
    ...
    const plainRequest = basename(fullRequest, extname(fullRequest)); // Normalize requester between different OSs

    const normalizedRequester = relative(root, requester).split(sep).join("/"); // Disable hashes if `{ hashes: false }` option is provided
...
export default smartWebpackImport;
export { shouldPrintComment };
